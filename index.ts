function makeCoprime(divisor: bigint) {
  let coprime = divisor;
...
function makeRule(divisor: bigint) {
  let rule = 0n;
  switch (divisor % 10n) {
    case 1n:
      rule = -(divisor / 10n);
...
function reduce(dividend: bigint, divisor: bigint, rule: bigint) {
  let eqs: string = "<br>";
...
function combine(a: bigint, b: bigint, p: bigint, q: bigint) {
  let [s, t] = egcd(p, q);
  return (a * q * t + b * p * s) % (p * q);
}

function solve() {
  let string = (document.getElementById("divisor") as HTMLInputElement).value;
  let divisor = BigInt(string);
  string = (document.getElementById("dividend") as HTMLInputElement).value;
